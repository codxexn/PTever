...
//    1대1 문의 목록
    ...
    public String GoTopersonalFaqlists (HttpSession session, FaqVO faqVO, Pagination pagination, Model model) {
        if (session.getAttribute("user") == null) {
            return "/login/login";
        }
        UserVO userVO = (UserVO) session.getAttribute("user");
        log.info("{}", userVO.toString());
        pagination.setTotal(faqService.findTotalByUserId(userVO.getUserId()));
        pagination.progress();
        List<FaqVO> faqlist = faqService.findAllByUserId(pagination, userVO.getUserId());
