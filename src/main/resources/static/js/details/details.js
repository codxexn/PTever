...
// 게시물 수정 클릭 시 실행 함수
const updatePostId = document.querySelector(".detailContent-UpdateButton input");
function updatePost(seq) {
    ...
    obj.setAttribute('value', updatePostId.value);

    f.appendChild(obj);
    f.setAttribute('method', 'post');
    f.setAttribute('action', '/community/detail-update');
...
// 댓글 삭제 버튼 클릭 시 실행 함수
const communityCommentId = document.querySelector(".detailContent-comment-DeleteButton input");
function deleteComment(seq){
    Swal.fire({
        ...
        if (result.value) {
            ...
            obj = document.createElement('input');
            obj.setAttribute('type', 'hidden');
            obj.setAttribute('name', 'communityCommentId');
            obj.setAttribute('value', communityCommentId.value);

            let obj2;
            obj2 = document.createElement('input');
            obj2.setAttribute('type', 'hidden');
            obj2.setAttribute('name', 'postId');
            obj2.setAttribute('value', postId.value);

            f.appendChild(obj);
            f.appendChild(obj2);
            f.setAttribute('method', 'post');
            f.setAttribute('action', '/community/delete-comment');
            document.body.appendChild(f);
            f.submit();
        }
    })
